...
async def run_daily(date_compact: str) -> PipelineContext:
    """Run the conference paper pipeline, return context with HTML+MD fragments."""
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Ascan 会议论文追踪")
    parser.add_argument("--date", "-d", help="日期 YYYYMMDD（默认：今天）")
    parser.add_argument("--init-db", action="store_true", help="初始化数据库")
    args = parser.parse_args()

    setup_logging("ascan_conf", get_settings().log_level)
    ...
    if context.conference_html and "empty-state" not in context.conference_html:
        logger.success(f"会议论文 pipeline 完成 ({len(context.conference_html)} chars)")
    else:
        logger.info("会议论文 pipeline 未产出内容（近期无新论文）")
