...
async def run_daily(date_compact: str) -> PipelineContext:
    """Run the official tracker pipeline, return context with HTML+MD fragments."""
    ...
    stage_enums = [Stage.FETCHING, Stage.PARSING, Stage.ANALYZING, Stage.GENERATING]
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Ascan 官方动态跟踪")
    parser.add_argument("--date", "-d", help="日期 YYYYMMDD（默认：今天）")
    parser.add_argument("--init-db", action="store_true", help="初始化数据库")
    args = parser.parse_args()

    setup_logging("ascan_official", get_settings().log_level)
    ...
    if context.official_html:
        logger.success(f"官方动态 pipeline 完成 ({len(context.official_html)} chars)")
    else:
        logger.info("官方动态 pipeline 未产出内容")
