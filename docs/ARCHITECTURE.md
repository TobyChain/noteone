...
routes/
  auth.ts            POST /auth/local · /auth/dev-token
...
`npm run mcp`（`tsx src/mcp.ts`）启动，stdio transport。用户身份：优先 `MCP_USER_ID` 环境变量，未设时自动取库中首个用户（需 App 至少启动过一次）。
...
### 7.2 历史：独立 MCP（`mcp-server/`）—— HTTP 代理

早期为外部 AI 提供 HTTP 代理版 MCP（`NOTEONE_API_URL` + `NOTEONE_TOKEN`，5 只读工具），已删除。现在统一使用 7.1 的内嵌 MCP（直连 DB，工具更全）；远程部署场景如需 HTTP 代理，可经 `POST /api/*` REST 接口自行包装。
...
| `MCP_USER_ID` | 否 | "" | 覆盖自动解析的笔记所有者 UUID |
