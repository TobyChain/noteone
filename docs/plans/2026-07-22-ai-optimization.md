...
| 2 | Ascan 6 模块串行执行 | `runner.ts:98-114` | 总耗时 = 6 模块之和（6-30 分钟） |
...
→ Phase 4（Ascan 并行）→ Phase 2.1（SSE 流式）→ Phase 2.4（Doom-loop）
...
- Phase 4：同一日期跑 Ascan，对比串行 vs 并行总耗时（期望 ≈ 最慢单模块）
