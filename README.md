...
壹识是一个以 AI 为内核的个人知识系统。随手捕获所见所闻，AI 静默整理、打标、向量化；闹闹（Notty）作为核心 Agent，可调度本地终端、定时任务、新知补充等工具；新知引擎每日扫遍 arXiv、GitHub、官方博客、独立笔耕、会议论文与微信公众号，汇成一份科技前沿日报。MCP 协议让 Claude、Cursor 等外部 AI 直连你的笔记库，如遇故人。
...
| **往事（笔记）** | 时间分组列表，语义搜索，标签筛选。macOS 侧边栏头部"+"按钮 / iOS 工具栏"+"按钮一键新建笔记。每条笔记附 AI 摘要、来源、作者、标签胶囊 |
| **Notty 对话（闹闹）** | 三层上下文管理（token 预算裁剪、渐进式摘要、保护区）、doom-loop 检测、工具调用持久化、Markdown 渲染。可调用本地终端白名单命令、定时任务、新知补充等工具 |
| **新知（Ascan）** | 每日自动运行 Python pipeline，抓取 arXiv 论文、GitHub 热门项目、官方研究动态、独立博客、会议论文、微信公众号，LLM 筛选翻译，生成带大纲导航的 HTML 日报。闹闹可作为编排者逐模块调度 |
| **定时任务** | 闹闹可通过自然语言创建 cron 定时任务（如"每天 8 点补充新知"），DB 持久化 + 服务启动时自动恢复 |
...
```
  ...
  │  ascan (reports / config / trigger / status / wechat-health)   │
...
壹识是开源项目，不内置 LLM 服务。所有 AI 功能（打标、摘要、Notty 聊天、报告、新知日报）需要你自带 API Key。打开「设置 → AI 模型」：

| 字段 | 示例 |
|---|---|
| API Key | 你的 OpenAI / DashScope / 自部署 vLLM 的 key |
| Base URL | `https://dashscope.aliyuncs.com/compatible-mode/v1` 或 `https://api.openai.com/v1`（填到版本号即可，系统自动拼接 `/chat/completions` 和 `/embeddings`） |
...
在「设置 → 新知」中配置科技日报的爬取参数：ArXiv 分类、GitHub topics、论文数量上限、会议等级过滤、博客源、微信公众号等。微信公众号抓取基于 [wechat-article-exporter](https://github.com/wechat-article/wechat-article-exporter)（原生 Node.js 部署，无需 Docker），部署步骤详见 [docs/wechat-wae-setup.md](docs/wechat-wae-setup.md)。点击「运行」或跟闹闹说"补充今日新知"即可触发 pipeline 生成当日日报。
...
- 闹闹本地终端工具走白名单 + 限定目录（`~/Documents` `~/Desktop` `~/Downloads`），屏蔽 shell 元字符
...
│   └── README.md           #   后端说明
├── ascan/                  # 新知 Python Pipeline（arXiv/GitHub/博客/会议/微信）
...
NoteOne is an AI-powered personal knowledge system. Capture anything from anywhere; AI silently tags, summarizes, and embeds it. Chat with Notty (闹闹), your core AI agent that can run local terminal commands, schedule tasks, and orchestrate the Ascan pipeline. The Ascan engine scans arXiv, GitHub, official blogs, conference papers, and WeChat public accounts daily, producing a curated tech-frontier report with table-of-contents navigation. MCP protocol lets Claude / Cursor talk directly to your note database.
...
| Notes (往事) | Time-grouped list, semantic search, tag filter, "+" button to create new notes, AI summary cards, tag pills |
| Notty Chat (闹闹) | Three-layer context management, doom-loop detection, tool call persistence, Markdown rendering. Tools: local terminal (whitelist), scheduled tasks, Ascan supplement, web search, notes retrieval |
| Ascan (新知) | Daily Python pipeline: arXiv, GitHub, official blogs, RSS, conference papers, WeChat (via wechat-article-exporter). HTML report with TOC navigation. Notty can orchestrate per-module |
| Scheduled Tasks | Notty creates cron tasks via natural language (e.g. "supplement new knowledge every 8am"), DB-persisted, auto-restored on server boot |
...
| Ascan | `GET /api/ascan/reports`, `GET /api/ascan/reports/:date`, `GET/PATCH /api/ascan/config`, `POST /api/ascan/trigger`, `POST /api/ascan/run-module`, `POST /api/ascan/merge`, `POST /api/ascan/abort`, `GET /api/ascan/status`, `GET /api/ascan/wechat-health`, `POST /api/ascan/summarize` |
