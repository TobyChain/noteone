...
struct NewLoreReportMeta: Identifiable, Decodable, Hashable {
    var id: String { date }
    let date: String
    let filename: String
    let size: Int
...
struct NewLoreConfig: Codable, Hashable {
...
struct NewLoreSupplementProgress: Decodable, Hashable {
    let isRunning: Bool
    let date: String
    let startedAt: String?
    let phase: String  // running | merging | done | failed
    let modules: [NewLoreModuleProgress]
    ...
    var failedModules: [NewLoreModuleProgress] {
...
struct NewLoreRunStatus: Decodable {
    let isRunning: Bool
    let pid: Int?
    ...
    let supplement: NewLoreSupplementProgress?
    let studyReport: StudyReportStatus?
}

struct NewLoreTriggerResponse: Decodable {
    let pid: Int
    let message: String
}

struct NewLoreReportResponse: Decodable {
    let date: String
    let html: String
}

struct NewLoreReportsResponse: Decodable {
    let reports: [NewLoreReportMeta]
}
