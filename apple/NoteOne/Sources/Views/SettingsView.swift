...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        .alert(L("发现新版本", "New Version Available"), isPresented: $showUpdateAlert, presenting: updateInfo) { info in
            ...
            Button(L("查看发布页", "Release Page")) {
                if let url = URL(string: info.releasePage) {
                    #if os(macOS)
                    NSWorkspace.shared.open(url)
                    #else
                    UIApplication.shared.open(url)
            ...
            Button(L("稍后提醒", "Later"), role: .cancel) {}
        } message: { info in
            Text(L("已发布 v", "Released v") + info.version + L("。下载后拖入 Applications 覆盖即可，你的数据不受影响。", ". Drag into Applications to install; your data is unaffected."))
        }
        .fileImporter(
        ...
        Section {
            ...
            Button {
                ...
                HStack {
                    if isCheckingUpdate {
                        ProgressView().controlSize(.small)
                        Text(L("正在检查…", "Checking…"))
                    } else {
                        Label(L("检查更新", "Check for Updates"), systemImage: "arrow.triangle.2.circlepath")
                    }
                }
            }
            .disabled(isCheckingUpdate)
            if let updateMessage {
                Text(updateMessage).font(.caption).foregroundStyle(Color.inkSecondary)
            }
    ...
    private func checkForUpdate() async {
        ...
        do {
            ...
            await MainActor.run {
                isCheckingUpdate = false
                if case UpdateError.noNewerVersion = error {
                    updateMessage = L("当前已是最新版本 v", "You're on the latest version v") + UpdateChecker.shared.currentVersion
                } else {
                    updateMessage = L("检查更新失败，请稍后重试", "Update check failed, try again later")
                }
            }
        }
    }
