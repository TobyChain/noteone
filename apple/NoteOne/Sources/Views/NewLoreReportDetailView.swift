...
struct NewLoreReportDetailView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            NewLoreWebView(htmlContent: htmlContent) { wv in
                webView = wv
            }
            .overlay(alignment: .bottom) {
    ...
    private var addToNotesBanner: some View {
        let (icon, text, color): (String, String, Color) = {
            ...
            case .saving: return ("arrow.2.circle", L("正在保存到往事…", "Saving to OldEcho…"), Color.accent)
            case .success: return ("checkmark.circle.fill", L("已添加到往事", "Added to OldEcho"), Color.success)
...
#if os(iOS)
struct NewLoreWebView: UIViewRepresentable {
    let htmlContent: String
...
#elseif os(macOS)
struct NewLoreWebView: NSViewRepresentable {
    let htmlContent: String
    var onWebViewReady: ((WKWebView) -> Void)? = nil
