// 新知配置页 — 由 App WebView 以 /newlore/?token={JWT} 打开。
// 配置读写走 /api/newlore/config（需 JWT）。
(() => {
  ...
  async function loadConfig() {
    config = await api("/api/newlore/config");
    renderAll();
  }

  async function loadPresets() {
    const data = await api("/api/newlore/presets");
    ...
    for (const p of data.presets) {
      ...
      btn.onclick = async () => {
        ...
        try {
          await api("/api/newlore/config/preset", {
            method: "POST",
  ...
  function renderAll() {
    ...
    $("farview-minimum-count").value = config.farview_minimum_count ?? 2;
    $("farview-blocked-topics").value = (config.farview_blocked_topics || []).join(", ");
  ...
  async function saveAll() {
    const updates = {
      ...
      farview_minimum_count: parseInt($("farview-minimum-count").value, 10) || 2,
      farview_blocked_topics: splitList($("farview-blocked-topics").value),
