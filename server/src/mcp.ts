...
// Lazy-resolved local user id. Honors MCP_USER_ID for backward compat; otherwise
// resolves to the first existing user (same rule the /auth/local endpoint uses).
// Requires the app to have been launched once so a user row exists.
let _userId: string | null = null;
async function getUserId(): Promise<string> {
  if (_userId) return _userId;
  if (process.env.MCP_USER_ID) return _userId = process.env.MCP_USER_ID;
  const u = await db.query.users.findFirst({ orderBy: asc(users.createdAt) });
  if (!u) throw new Error("No local user yet — launch the NoteOne app once to create one");
  return _userId = u.id;
}
...
server.tool(
  ...
  async ({ content, source_url, title, source_app }) => {
    const { id } = await mcpCreateNote(await getUserId(), { content, source_url, title, source_app });
...
server.tool(
  ...
  async ({ id }) => {
    ...
    const [note] = await db.update(notes)
      ...
      .where(and(eq(notes.id, id), eq(notes.userId, userId), ne(notes.status, "trashed")))
      .returning();
    if (!note) return { content: [{ type: "text" as const, text: "笔记不存在" }] };
...
server.tool(
  ...
  async ({ id }) => {
    ...
    const [note] = await db.update(notes)
      ...
      .where(and(eq(notes.id, id), eq(notes.userId, userId), eq(notes.status, "trashed")))
      .returning();
...
server.tool(
  ...
  async ({ dimension }) => {
    ...
    const where = dimension
      ? and(eq(tags.userId, userId), eq(tags.dimension, dimension))
...
server.tool(
  ...
  { date: z.string().optional().describe("报告日期 YYYYMMDD，默认今天") },
  async ({ date }) => {
    const r = await mergeReport(date, await getUserId());
    return {
      content: [{
        type: "text" as const,
