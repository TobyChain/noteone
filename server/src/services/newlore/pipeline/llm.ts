 ...
 * Ported from newlore/src/tools/call_llm.py.
 */
...
export class PipelineLLM {
  ...
  private stats = { promptTokens: 0, completionTokens: 0, cachedTokens: 0, calls: 0 };
  ...
  /** Accumulate per-call usage. cached = DeepSeek prompt_cache_hit_tokens or OpenAI cached_tokens. */
  private recordUsage(usage: any): void {
    ...
    this.stats.completionTokens += completion;
    this.stats.cachedTokens += cached;
    this.stats.calls += 1;
    if (prompt > 0) {
      console.log(
        `[newlore-llm] in=${prompt} cached=${cached}/${prompt} out=${completion}`,
      );
    }
  }

  /** Aggregate usage since construction (or last reset). Callers log this per module. */
  getUsageStats(): { promptTokens: number; completionTokens: number; cachedTokens: number; calls: number } {
    return { ...this.stats };
  }

  resetUsageStats(): void {
    this.stats = { promptTokens: 0, completionTokens: 0, cachedTokens: 0, calls: 0 };
  }
      ...
      for (let attempt = 0; attempt < this.cfg.maxRetries; attempt++) {
        ...
        try {
          ...
          if (resp.status === 429) {
            ...
            console.warn(`[newlore-llm] 429 rate limited, waiting ${wait}ms (${attempt + 1}/${this.cfg.maxRetries})`);
            await sleep(wait);
          ...
          if (!String(err).includes("429")) {
            console.warn(`[newlore-llm] call failed (attempt ${attempt + 1}/${this.cfg.maxRetries}): ${err}`);
          }
        } finally {
          clearTimeout(timer);
        }
  ...
  async chatJsonRetry<T = any>(
    ...
    for (let i = 0; i < attempts; i++) {
      try {
        ...
        lastError = err;
        console.warn(`[newlore-llm] chatJsonRetry attempt ${i + 1}/${attempts} failed: ${err}`);
      }
    }
