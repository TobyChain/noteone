/**
 * GitHub module — TS port of newlore/src/github_agent/ (daily only).
...
import { newloreGithubRepos } from "../../../../db/schema.js";
...
async function llmFilterRepos(repos: RepoInfo[], ctx: ModuleContext): Promise<RepoInfo[]> {
  ...
  const schema = `你是一位技术编辑。请为以下 GitHub 仓库评估技术相关性和关注度，打分1-10分。

${topicsHint}${preferenceHint(ctx.preferences, ctx.language)}
...
async function getAllAnalyzedNames(): Promise<Set<string>> {
  const rows = await db
    .select({ fullName: newloreGithubRepos.fullName })
    .from(newloreGithubRepos)
    .where(and(eq(newloreGithubRepos.analyzed, true), isNotNull(newloreGithubRepos.oneLiner)));
...
async function getByFullNames(fullNames: string[]) {
  if (!fullNames.length) return new Map<string, typeof newloreGithubRepos.$inferSelect>();
  const rows = await db
    .select()
    .from(newloreGithubRepos)
    .where(inArray(newloreGithubRepos.fullName, fullNames));
...
async function upsertRepos(repos: RepoInfo[], today: string): Promise<void> {
  ...
  if (newRows.length) {
    await db.insert(newloreGithubRepos).values(newRows);
  }
  ...
  for (const repo of repos) {
    ...
    await db
      ...
      .where(eq(newloreGithubRepos.fullName, repo.full_name));
  }
}
...
async function saveAnalysis(fullName: string, analysis: RepoAnalysis): Promise<void> {
  await db
    ...
    .where(eq(newloreGithubRepos.fullName, fullName));
}

// ── analyzer (analyzer.py) ────────────────────────────────────────────────

/** Build a compact JSON prompt for repo analysis (verbatim port of _build_repo_prompt, restructured for prompt-cache). */
function buildRepoPrompt(repo: RepoInfo, language: "zh" | "en"): ChatMessages {
  ...
  return buildCachedPrompt(schema, variable);
}
