/**
 * Official tracker module — TS port of newlore/src/official_tracker/
...
import { newloreOfficialItems } from "../../../../db/schema.js";
...
// settings.py defaults (not part of the TS NewLoreConfig surface)
...
type OfficialRow = typeof newloreOfficialItems.$inferSelect;

async function getBySlug(slug: string): Promise<OfficialRow | undefined> {
  const rows = await db.select().from(newloreOfficialItems).where(eq(newloreOfficialItems.slug, slug)).limit(1);
  return rows[0];
}
...
async function getAllKnownSlugs(): Promise<Map<string, string | null>> {
  const rows = await db
    .select({ slug: newloreOfficialItems.slug, sitemapLastmod: newloreOfficialItems.sitemapLastmod })
    .from(newloreOfficialItems);
...
async function upsertDiscovered(entry: SitemapEntry, today: string): Promise<void> {
  await db
    ...
    .onConflictDoNothing({ target: newloreOfficialItems.slug });
}
...
async function saveScrapedContent(item: OfficialItem): Promise<void> {
  ...
  await db
    ...
    .where(eq(newloreOfficialItems.slug, item.slug));
}
...
async function saveAnalysis(slug: string, analysis: OfficialAnalysis): Promise<void> {
  await db
    ...
    .where(eq(newloreOfficialItems.slug, slug));
}
...
async function getAllAnalyzedSlugs(): Promise<Set<string>> {
  const rows = await db
    .select({ slug: newloreOfficialItems.slug })
    .from(newloreOfficialItems)
    .where(and(eq(newloreOfficialItems.analyzed, true), isNotNull(newloreOfficialItems.oneLiner)));
  return new Set(rows.map((r) => r.slug));
}

// ── LLM analysis (analyzer.py) ────────────────────────────────

function buildArticlePrompt(item: OfficialItem, language: "zh" | "en", preferences?: ModuleContext["preferences"]): ChatMessages {
  const contentPreview = (item.content || item.summary || "").slice(0, 1500);

  const schema =
    language === "en"
      ? `You are a tech content editor. Generate a summary for the following technical article, output in JSON format.
...
Output JSON only, no other content.`
      : `你是一位技术内容编辑。请为以下技术文章生成中文摘要，输出JSON格式。
...
只输出 JSON，不要其他内容。`;
  ...
  return buildCachedPrompt(schema + preferenceHint(preferences, language), variable);
}
...
async function analyzeItem(ctx: ModuleContext, item: OfficialItem): Promise<OfficialAnalysis | null> {
  try {
    ...
    const data = await ctx.llm.chatJsonRetry<Record<string, unknown>>(buildArticlePrompt(item, lang, ctx.preferences));
    return {
