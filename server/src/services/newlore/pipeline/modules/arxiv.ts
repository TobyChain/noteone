 ...
 * (newlore/main.py run_multi_dimension_pipeline + newlore/src/pipeline/stages.py).
 *
...
// ── constants (from newlore/src/config/settings.py) ─────────────────────────
...
function scoreStage(ctx: ModuleContext, papers: RawPaper[]): MultiDimensionScore[] {
  ...
  const preferenceTerms = `${ctx.preferences?.focus ?? ""},${ctx.preferences?.topics ?? ""}`
    .toLowerCase().split(/[,，;；\n]/).map((value) => value.trim()).filter((value) => value.length >= 2);
  const paperText = new Map(papers.map((paper) => [
    paper.arxivId,
    `${paper.title} ${paper.abstract}`.toLowerCase(),
  ]));
  const preferenceBoost = (paper: MultiDimensionScore) => {
    const text = paperText.get(paper.arxivId) ?? "";
    return preferenceTerms.some((term) => text.includes(term)) ? 5 : 0;
  };
  const selected = [...relevant]
    .sort((a, b) => (b.overallScore + preferenceBoost(b)) - (a.overallScore + preferenceBoost(a)))
...
/** Prompt — verbatim port of call_llm.LLMClient._create_analysis_prompt, restructured for prompt-cache. */
function createAnalysisPrompt(title: string, abstract: string, language: "zh" | "en"): ChatMessages {
  ...
  return buildCachedPrompt(schema, `PAPER TITLE: ${titleSafe} | ABSTRACT: ${abstractSafe}`);
}
  ...
  await db
    ...
    .onConflictDoUpdate({
      ...
      set: {
        ...
        pdfUrl: paper.pdfUrl,
        published,
        // Preserve the original observation date across later re-analysis.
        updatedAt: new Date(),
        ...analysisFields,
      },
    });
...
async function analyzeStage(
  ...
  for (let i = 0; i < scoredPapers.length; i++) {
    ...
    await upsertPaper(paper, dataDate, ctx.date, paperAnalysis);
  }
...
async function generateReport(
  ...
  const rows = selectedIds.length
    ? await db
        .select()
        .from(newlorePapers)
        .where(inArray(newlorePapers.arxivId, selectedIds))
    : await db
        .select()
        .from(newlorePapers)
        .where(eq(newlorePapers.published, dataDate))
        .limit(500);
...
export async function run(ctx: ModuleContext): Promise<ModuleResult> {
  ...
  try {
    const rows = await db.select({ arxivId: newlorePapers.arxivId }).from(newlorePapers);
