 ...
 * TS port of newlore/src/blog_subs/ (rss_parser.py / analyzer.py / report.py / stages.py / models.py).
 */
import Parser from "rss-parser";
import { eq } from "drizzle-orm";
import { db } from "../../../../db/client.js";
import { newloreBlogPosts, newloreOfficialItems } from "../../../../db/schema.js";
...
async function getAllKnownSlugs(): Promise<Set<string>> {
  const rows = await db.select({ slug: newloreBlogPosts.slug }).from(newloreBlogPosts);
...
async function getAllOfficialUrls(): Promise<Set<string>> {
  const rows = await db.select({ url: newloreOfficialItems.url }).from(newloreOfficialItems);
...
async function saveAnalysis(slug: string, analysis: BlogAnalysis): Promise<void> {
  await db
    ...
    .where(eq(newloreBlogPosts.slug, slug));
}

// ── stage 2: LLM analysis (port of analyzer.py) ───────────────

function buildBlogPrompt(post: BlogPost, language: "zh" | "en", preferences?: ModuleContext["preferences"]): ChatMessages {
  const contentPreview = (post.summary || "").slice(0, 1500);

  const schema =
    language === "en"
      ? `You are a tech content editor. Generate a summary for the following blog article, output in JSON format.
...
Output JSON only, no other content.`
      : `你是一位技术内容编辑。请为以下技术博客文章生成中文摘要，输出JSON格式。
...
async function analyzePost(ctx: ModuleContext, post: BlogPost): Promise<BlogAnalysis | null> {
  try {
    ...
    const data = await ctx.llm.chatJsonRetry<Record<string, any>>(buildBlogPrompt(post, lang, ctx.preferences));
    return {
