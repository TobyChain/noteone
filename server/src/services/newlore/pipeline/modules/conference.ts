/**
 * Conference paper tracking module — TS port of newlore/src/conf_tracker/
...
import { newloreConferencePapers, newlorePapers } from "../../../../db/schema.js";
import { sleep, fetchWithTimeout, escapeHtml, DEFAULT_UA } from "../util.js";
import { RECOMMENDATION_ORDER } from "../scoring.js";
import { buildCachedPrompt, preferenceHint, type ChatMessages } from "../prompts.js";
...
const CCF_YAML_URL = process.env.NEWLORE_DATA_DIR
  ? new URL(`file://${process.env.NEWLORE_DATA_DIR}/ccf_conferences.yaml`)
  : new URL("../data/ccf_conferences.yaml", import.meta.url);

// Defaults mirroring newlore/src/config/settings.py (these keys are not part of
...
async function llmFilterPapers(papers: ConferencePaper[], ctx: ModuleContext): Promise<ConferencePaper[]> {
  ...
  const schema = `你是一位 AI 领域的学术编辑。请为以下会议论文评估技术相关性和阅读价值，打分1-10分。${preferenceHint(ctx.preferences, ctx.language)}
...
请严格输出JSON（不要包含markdown代码块标记）：
{
  ...
  try {
    const data = await ctx.llm.chatJson<any>(buildCachedPrompt(schema, listStr));
...
const SCHEMA_ANALYSIS_ZH = `你是一位 AI 领域的学术论文分析专家。请分析以下会议论文，生成中文摘要和评估。
...
const SCHEMA_ANALYSIS_EN = `You are an expert academic paper analyst in the AI field. Analyze the following conference paper and generate a summary and assessment.
...
function buildPrompt(paper: ConferencePaper, language: "zh" | "en"): ChatMessages {
  const schema = language === "en" ? SCHEMA_ANALYSIS_EN : SCHEMA_ANALYSIS_ZH;
  const authors = paper.authors.slice(0, 5).join(", ") + (paper.authors.length > 5 ? "..." : "");
  const abstract = paper.abstract || (language === "en" ? "(No abstract)" : "（无摘要）");
  const userInfo =
    language === "en"
      ? `Paper info:\n- Title: ${paper.title}\n- Venue: ${paper.venue} (Rank ${paper.rank}, ${paper.category})\n- Authors: ${authors}\n- Abstract: ${abstract}`
      : `论文信息：\n- 标题：${paper.title}\n- 会议：${paper.venue} (${paper.rank}类, ${paper.category})\n- 作者：${authors}\n- 摘要：${abstract}`;
  return buildCachedPrompt(schema, userInfo);
}
...
async function getAnalyzedKeys(): Promise<Set<string>> {
  const rows = await db
    .select({ paperKey: newloreConferencePapers.paperKey })
    .from(newloreConferencePapers)
    .where(and(eq(newloreConferencePapers.analyzed, true), isNotNull(newloreConferencePapers.oneLiner)));
...
async function getArxivDois(): Promise<Set<string>> {
  const rows = await db
    .select({ doi: newlorePapers.doi })
    .from(newlorePapers)
    .where(and(isNotNull(newlorePapers.doi), ne(newlorePapers.doi, "")));
...
async function upsertDiscovered(paper: ConferencePaper, today: string): Promise<void> {
  await db
    .insert(newloreConferencePapers)
    .values({
    ...
    .onConflictDoUpdate({
      target: newloreConferencePapers.paperKey,
...
async function saveAnalysis(paperKey: string, analysis: ConferenceAnalysis): Promise<void> {
  await db
    .update(newloreConferencePapers)
    .set({
    ...
    .where(eq(newloreConferencePapers.paperKey, paperKey));
}
...
async function loadCachedAnalyses(
  ...
  const rows = await db
    .select()
    .from(newloreConferencePapers)
    .where(inArray(newloreConferencePapers.paperKey, cachedKeys));
