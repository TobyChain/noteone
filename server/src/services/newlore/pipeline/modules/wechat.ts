/**
 * WeChat MP module — TS port of newlore/src/wechat_tracker/
...
import { newloreWechatArticles } from "../../../../db/schema.js";
...
async function getAllKnownIds(): Promise<Set<string>> {
  const rows = await db.select({ articleId: newloreWechatArticles.articleId }).from(newloreWechatArticles);
  return new Set(rows.map((r) => r.articleId));
}

async function upsertDiscovered(article: WeChatArticle, today: string): Promise<void> {
  await db
    .insert(newloreWechatArticles)
    .values({
    ...
    .onConflictDoUpdate({
      target: newloreWechatArticles.articleId,
...
async function saveAnalysis(articleId: string, analysis: WeChatAnalysis): Promise<void> {
  await db
    ...
    .where(eq(newloreWechatArticles.articleId, articleId));
}

// ── LLM analysis (analyzer.py) ────────────────────────────────

function buildPrompt(article: WeChatArticle, language: "zh" | "en"): ChatMessages {
  ...
  const schema =
    language === "en"
      ? `You are an AI content analysis expert. Analyze the following WeChat article and generate a summary and assessment.
...
Output strictly as JSON (no markdown code block markers):
{
  ...
  "relevance": "Highly Recommended/Recommended/Worth Reading/Moderately Recommended/Not Recommended"
      ...
      : `你是一位 AI 领域的内容分析专家。请分析以下微信公众号文章，生成中文摘要和评估。
...
async function llmFilterArticles(
  ...
  const schema = `你是一位科技日报编辑。请为以下微信公众号文章评估技术相关性和阅读价值，打分1-10分。${preferenceHint(ctx.preferences, ctx.language)}
...
请严格输出JSON（不要包含markdown代码块标记）：
{
  ...
  try {
    const data = await ctx.llm.chatJson<any>(buildCachedPrompt(schema, `文章列表：\n${articleListStr}`));
