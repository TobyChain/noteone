/**
 * Shared prompt infrastructure for the newlore pipeline.
 *
...
/** Personal preferences are a soft ranking signal, never a hard exclusion rule. */
export function preferenceHint(preferences?: { focus?: string; topics?: string }, language: "zh" | "en" = "zh"): string {
  const focus = preferences?.focus?.trim();
  const topics = preferences?.topics?.trim();
  if (!focus && !topics) return "";
  if (language === "en") {
    return `\nUser preference (soft ranking signal only; do not discard important global discoveries): daily focus=${focus || "none"}; long-term topics=${topics || "none"}.`;
  }
  return `\n用户偏好（仅用于软排序，不能因此淘汰重要的全局新发现）：今日重点=${focus || "无"}；长期兴趣=${topics || "无"}。`;
}
