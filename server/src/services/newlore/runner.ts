/**
 * NewLore runner — drives the in-process TS pipeline.
 *
 * - startNewLoreSupplement: non-blocking, runs all modules + merge in background,
...
import { startFarViewRefresh } from "../farview/service.js";
import type { NewLoreModuleName } from "./pipeline/types.js";

export interface NewLoreRunStatus {
  isRunning: boolean;
  pid: number | null;
...
function freshProgress(date: string, language: "zh" | "en" = "zh", activeModules?: string[]): SupplementProgress {
  ...
  const mods = (activeModules && activeModules.length > 0 ? activeModules : ALL_MODULES)
    .filter((m) => ALL_MODULES.includes(m as NewLoreModuleName));
  return {
    isRunning: false,
    date,
    startedAt: null,
    phase: "running",
    modules: mods.map((m) => ({
      name: m, label: labels[m as NewLoreModuleName] ?? m, status: "pending" as const, chars: 0, error: null,
    })),
    currentModule: null,
    error: null,
  };
...
export async function runModule(
  ...
  console.log(`[newlore] runModule ${module} date=${dateStr} (in-process)`);
...
export async function mergeReport(
  ...
  const dateStr = date || todayDateStr();
  console.log(`[newlore] mergeReport date=${dateStr} (in-process)`);
...
async function runSupplement(dateStr: string, llmConfig?: LLMOverride, userId?: string, activeModules?: string[]): Promise<void> {
  ...
  const results = await Promise.allSettled(
    modulesToRun.map(async (mod) => {
      ...
      try {
        ...
        console.log(`[newlore] supplement ${mod}: ${mp.status} (${mp.chars} chars)`);
      } catch (err: any) {
        mp.status = "failed";
        mp.error = err?.message || String(err);
        console.error(`[newlore] supplement ${mod} exception:`, err);
      }
    }),
  );
  ...
  for (const r of results) {
    if (r.status === "rejected" && r.reason?.message !== "aborted") {
      console.error("[newlore] supplement unexpected rejection:", r.reason);
    }
  }

  {
    const u = sharedLlm.getUsageStats();
    console.log(
      `[newlore] supplement LLM usage: calls=${u.calls} in=${u.promptTokens} cached=${u.cachedTokens}/${u.promptTokens} out=${u.completionTokens}`,
    );
  }
  ...
  try {
    ...
    if (r.ok) {
      try {
        ...
        console.log(`[newlore] supplement summary: ${summary.slice(0, 60)}`);
      } catch (err: any) {
        console.error(`[newlore] supplement summary failed:`, err);
      }
      try {
        startFarViewRefresh(`${dateStr.slice(0, 4)}-${dateStr.slice(4, 6)}-${dateStr.slice(6, 8)}`);
      } catch (err: any) {
        if (err?.status !== 409) console.error("[farview] refresh trigger failed:", err);
      }
    }
  } catch (err: any) {
  ...
  supplementProgress!.isRunning = false;
  supplementProgress!.currentModule = null;
  console.log(`[newlore] supplement finished: ${supplementProgress!.phase}`);
}

/**
 * Start the full newlore supplement in the background. Returns immediately so
 * the chat request is not blocked. Progress is tracked in `supplementProgress`
 * and exposed via `getRunStatus()`.
 */
export async function startNewLoreSupplement(
  date?: string,
  llmConfig?: LLMOverride,
  userId?: string,
  ...
  console.log(`[newlore] startNewLoreSupplement date=${dateStr} modules=${enabled.join(",")} (background, in-process)`);

  // Run in background — do NOT await.
  runSupplement(dateStr, llmConfig, userId, enabled)
    .catch((err) => {
      console.error("[newlore] supplement background error:", err);
      supplementProgress!.isRunning = false;
      supplementProgress!.phase = "failed";
  ...
  return { started: true, date: dateStr, modules: enabled.map((m) => labels[m as NewLoreModuleName] ?? m) };
}
...
export async function startNewLoreModules(
  ...
  const valid = modules.filter((m) => ALL_MODULES.includes(m as NewLoreModuleName));
  if (valid.length === 0) {
  ...
  console.log(`[newlore] startNewLoreModules date=${dateStr} modules=${valid.join(",")} (background)`);

  runSupplement(dateStr, llmConfig, userId, valid)
    .catch((err) => {
      console.error("[newlore] runModules background error:", err);
      supplementProgress!.isRunning = false;
      supplementProgress!.phase = "failed";
      supplementProgress!.error = String(err);
    })
  ...
  return { started: true, date: dateStr, modules: valid.map((m) => labels[m as NewLoreModuleName] ?? m) };
}
...
export async function triggerRun(
  ...
  const dateStr = date || todayDateStr();
  await startNewLoreSupplement(dateStr, llmConfig, userId);
  // pid is meaningless in-process; kept for API compatibility with old clients.
  return { pid: process.pid, message: `NewLore pipeline started (in-process, date: ${dateStr})` };
}
...
export async function abortRun(): Promise<{ killed: boolean; message: string }> {
  if (supplementProgress?.isRunning) {
    ...
    console.log("[newlore] abort requested — cancelling in-flight modules");
    return { killed: true, message: "Pipeline abort requested" };
  }
  return { killed: false, message: "No pipeline running" };
}

export async function getRunStatus(): Promise<NewLoreRunStatus> {
