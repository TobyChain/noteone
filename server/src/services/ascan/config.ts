/**
 * Ascan configuration — .env parsing/serialization for the NewSee pipeline.
 *
...
// Dev: runtime data lives at server/.ascan/ (this file is at
// server/src/services/ascan/, so three levels up).
// Embedded (packaged .app): everything lives under {DATA_DIR}/ascan.
const SERVER_ASCAN_ROOT = resolve(__dirname, "../../../.ascan");
const ASCAN_ROOT = appConfig.dataDir ? join(appConfig.dataDir, "ascan") : SERVER_ASCAN_ROOT;
...
const SCHEMA_PATH = process.env.ASCAN_SCHEMA_PATH || join(ASCAN_ROOT, "config.schema.json");
